...
pct_df_1X1Z <- calcPctIn(df_1X1Z)
# pct_df_2X2W2Z <- calcPctIn(df_2X2W2Z)

# Testing for near-equality -----------------------------------------------
# test_that("1 X", expect_gte(min(pct_df_1X$coverage), 0.9))
# test_that("2 X", expect_gte(min(pct_df_2X$coverage), 0.9))
test_that("1 binomial W", expect_gte(min(pct_df_1W$coverage), 0.9))
# test_that("2 binomial W", expect_gte(min(pct_df_2W$coverage), 0.9))
# test_that("1 polynomial Z", expect_gte(min(pct_df_1W$coverage), 0.9))
test_that("1 X and 1 W", expect_gte(min(pct_df_1X1W$coverage), 0.9))
# test_that("2 X, 2 W, 2 Z", expect_gte(min(pct_df_2W$coverage), 0.9))
