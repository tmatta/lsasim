...
test_that("scalar c_sd are correctly passed", {
  expect_equivalent(unique(diag(dataX$cov_matrix)[1]), sd_size_1)
  expect_equivalent(unique(diag(dataXW$cov_matrix)[1]), sd_size_1)
  expect_equivalent(unique(diag(dataYW$cov_matrix)[1]), sd_size_1)
  expect_equivalent(unique(diag(dataYXW$cov_matrix)[1:2]), sd_size_1)
})
...
test_that("vector c_sd are correctly passed", {
  expect_equivalent(diag(dataX$cov_matrix)[1:2], sd_size_2)
  expect_equivalent(diag(dataXW$cov_matrix)[1:2], sd_size_2)
  expect_equivalent(diag(dataYXW$cov_matrix)[1:2], sd_size_2)
})
